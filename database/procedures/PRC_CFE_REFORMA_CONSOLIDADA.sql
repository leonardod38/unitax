CREATE OR REPLACE PROCEDURE USER_XMLS.PRC_CFE_REFORMA_CONSOLIDADA AS
    CURSOR c_cfe IS
        SELECT /*+ PARALLEL(32) */
           'CFE' AS TIPO_DOCUMENTO,
           t.NOME_ARQUIVO,
           -- Limpando o prefixo "CFe" para manter apenas a chave numérica de 44 posições
           SUBSTR(capa.ID_CORTADO, 4, 44) AS CHAVE_ACESSO,

           -- Grupo B - Identificadores
           capa.B02_CUF,
           capa.B03_CNF,
           NULL AS B04_NATOP, -- CF-e não possui Natureza da Operação na capa
           capa.B06_MOD,
           capa.B07_SERIE,
           capa.NUMERO_NF,
           -- Concatenação de dEmi e hEmi
           TO_CHAR(TO_DATE(capa.DEMI_RAW || capa.HEMI_RAW, 'YYYYMMDDHH24MISS'), 'DD/MM/YYYY') AS DATA_EMISSAO_NFE,
           NULL AS B10_DHSAIENT, NULL AS B11_TPNF, NULL AS B11A_IDDEST, NULL AS B12_CMUNFG, NULL AS B21_TPIMP,
           capa.B22_TPEMIS, capa.B23_CDV, capa.B24_TPAMB, NULL AS B25_FINNFE, NULL AS B25A_INDFINAL,
           NULL AS B25B_INDPRES, NULL AS B25C_INDINTERMED, NULL AS B26_PROCEMI, NULL AS B27_VERPROC,
           ...
           capa.C05_XLGR, capa.C05_NRO, capa.C05_XCPL, capa.C05_XBAIRRO, NULL AS C05_CMUN,
           capa.C05_XMUN, NULL AS C05_UF, capa.C05_CEP, NULL AS C05_CPAIS, NULL AS C05_XPAIS, NULL AS C05_FONE,
           capa.C17_IE, NULL AS C18_IEST, capa.C19_IM, NULL AS C20_CNAE, capa.C21_CRT,

           -- 2. Destinatario Completo (No SAT, frequentemente nulos)
           capa.E02_CNPJ, capa.E03_CPF, NULL AS E03A_IDESTRANGEIRO, capa.E04_XNOME,
           NULL AS E05_XLGR, NULL AS E05_NRO, NULL AS E05_XCPL, NULL AS E05_XBAIRRO, NULL AS E05_CMUN,
           NULL AS E05_XMUN, NULL AS E05_UF, NULL AS E05_CEP, NULL AS E05_CPAIS, NULL AS E05_XPAIS, NULL AS E05_FONE,
           NULL AS E16_INDIEDEST, NULL AS E17_IE, NULL AS E18_ISUF, NULL AS E19_IM, NULL AS E20_EMAIL,
           ...
           NULL AS I12_CEANTRIB, NULL AS I13_UTRIB,
           NULL AS I14_QTRIB, NULL AS I14A_VUNTRIB,
           ROUND(TO_NUMBER(NVL(det.I15_VDESC_STR, '0'), '99999999999990.9999999999', 'NLS_NUMERIC_CHARACTERS = ''. '''), 2) AS I15_VDESC,
           det.I17B_INDTOT,

           -- 4. Impostos Atuais (ICMS) - Wildcards para englobar as N tags de CSOSN
           ...
           -- 5. IPI - Nulo para CF-e padrão
           NULL AS IPI_CENQ, NULL AS IPI_CNPJPROD, NULL AS IPI_CST,
           NULL AS IPI_VBC, NULL AS IPI_PIPI, NULL AS IPI_QUNID, NULL AS IPI_VUNID, NULL AS IPI_VIPI,

           -- 6. PIS e COFINS - Wildcards para Aliq e NT
           det.PIS_CST,
           ...
           NULL AS PISST_VBC, NULL AS PISST_PPIS, NULL AS PISST_QBCPROD, NULL AS PISST_VALIQPROD, NULL AS PISST_VPIS,
           ...
           NULL AS COFINSST_VBC, NULL AS COFINSST_PCOFINS, NULL AS COFINSST_QBCPROD, NULL AS COFINSST_VALIQPROD, NULL AS COFINSST_VCOFINS
        ...
        -- Filtro rígido na Staging para ler apenas as estruturas raízes de CF-e
    ...
    BEGIN
        ...
        VALUES (p_chave, 'PRC_CFE_REFORMA_CONSOLIDADA', p_nitem, p_cod, SUBSTR(p_msg, 1, 4000));
        COMMIT;
    END gravar_log;

BEGIN
    -- Limpa apenas o lote referente ao CF-e para não impactar a NF-e
    DELETE FROM TB_REFORMA_CONSOLIDADA WHERE TIPO_DOCUMENTO = 'CFE';
    COMMIT;
