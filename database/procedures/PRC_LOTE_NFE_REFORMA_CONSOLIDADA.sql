-- =============================================================================
-- Procedure..: USER_XMLS.PRC_LOTE_NFE_REFORMA_CONSOLIDADA
-- Resumo.....: Consolida NF-e enviadas em LOTE (XML raiz /enviNFe com N notas).
--              Le os XMLs da stg_nfe (filtro XMLExists em /enviNFe), extrai via
--              XMLTABLE (capa /enviNFe/NFe + itens) e grava na
--              TB_REFORMA_CONSOLIDADA como TIPO_DOCUMENTO = 'LOTE_NFE'
--              (DELETE seletivo, BULK COLLECT + FORALL SAVE EXCEPTIONS).
--
-- Historico:
--   v1.1.0 - 2026-06-15 - Adicionados os campos Z03_INFCPL (infNFe/infAdic/infCpl)
--                         e ZX02_QRCODE (infNFeSupl/qrCode): extracao no XMLTABLE
--                         da capa e gravacao no INSERT da TB_REFORMA_CONSOLIDADA.
--   v1.0.0 - 2026-05-19 - Versao inicial.
-- =============================================================================
CREATE OR REPLACE PROCEDURE USER_XMLS.PRC_LOTE_NFE_REFORMA_CONSOLIDADA AS
    CURSOR c_lote IS
        SELECT /*+ PARALLEL(32) */
           'LOTE_NFE' AS TIPO_DOCUMENTO,
           t.NOME_ARQUIVO,
           -- Extraindo os 44 numéricos do atributo Id="NFe3524..."
        ...
        FROM stg_nfe t,
             -- ==========================================
             -- XMLTABLE 1: CAPA (Pai) - Lê Lote de Envio
             -- ==========================================
             XMLTABLE (
                 XMLNAMESPACES (DEFAULT 'http://www.portalfiscal.inf.br/nfe'),
                 '/enviNFe/NFe' -- Descendo o cursor para iterar sobre N notas do Lote
                 PASSING XMLTYPE(t.conteudo_xml)
                 COLUMNS
                     ID_CORTADO       VARCHAR2(50) PATH 'infNFe/@Id',
                     NUMERO_NF        VARCHAR2(20) PATH 'infNFe/ide/nNF',
                     DATA_EMISSAO_RAW VARCHAR2(30) PATH 'infNFe/ide/dhEmi',

                     -- Grupo B
                     B02_CUF          VARCHAR2(2)  PATH 'infNFe/ide/cUF',
                     B03_CNF          VARCHAR2(8)  PATH 'infNFe/ide/cNF',
                     B04_NATOP        VARCHAR2(60) PATH 'infNFe/ide/natOp',
                     B06_MOD          VARCHAR2(2)  PATH 'infNFe/ide/mod',
                     B07_SERIE        VARCHAR2(3)  PATH 'infNFe/ide/serie',
                     B10_DHSAIENT     VARCHAR2(30) PATH 'infNFe/ide/dhSaiEnt',
                     B11_TPNF         VARCHAR2(1)  PATH 'infNFe/ide/tpNF',
                     B11A_IDDEST      VARCHAR2(1)  PATH 'infNFe/ide/idDest',
                     B12_CMUNFG       VARCHAR2(7)  PATH 'infNFe/ide/cMunFG',
                     B21_TPIMP        VARCHAR2(1)  PATH 'infNFe/ide/tpImp',
                     B22_TPEMIS       VARCHAR2(1)  PATH 'infNFe/ide/tpEmis',
                     B23_CDV          VARCHAR2(1)  PATH 'infNFe/ide/cDV',
                     B24_TPAMB        VARCHAR2(1)  PATH 'infNFe/ide/tpAmb',
                     B25_FINNFE       VARCHAR2(1)  PATH 'infNFe/ide/finNFe',
                     B25A_INDFINAL    VARCHAR2(1)  PATH 'infNFe/ide/indFinal',
                     B25B_INDPRES     VARCHAR2(1)  PATH 'infNFe/ide/indPres',
                     B25C_INDINTERMED VARCHAR2(1)  PATH 'infNFe/ide/indIntermed',
                     B26_PROCEMI      VARCHAR2(1)  PATH 'infNFe/ide/procEmi',
                     B27_VERPROC      VARCHAR2(20) PATH 'infNFe/ide/verProc',
                     B28_DHCONT       VARCHAR2(30) PATH 'infNFe/ide/dhCont',
                     B29_XJUST        VARCHAR2(256) PATH 'infNFe/ide/xJust',
                     B_CMUNFGIBS      VARCHAR2(7)  PATH 'infNFe/ide/cMunFGIBS',

                     -- Emitente
                     C02_CNPJ         VARCHAR2(14) PATH 'infNFe/emit/CNPJ',
                     C02A_CPF         VARCHAR2(11) PATH 'infNFe/emit/CPF',
                     C03_XNOME        VARCHAR2(60) PATH 'infNFe/emit/xNome',
                     C04_XFANT        VARCHAR2(60) PATH 'infNFe/emit/xFant',
                     C05_XLGR         VARCHAR2(60) PATH 'infNFe/emit/enderEmit/xLgr',
                     C05_NRO          VARCHAR2(60) PATH 'infNFe/emit/enderEmit/nro',
                     C05_XCPL         VARCHAR2(60) PATH 'infNFe/emit/enderEmit/xCpl',
                     C05_XBAIRRO      VARCHAR2(60) PATH 'infNFe/emit/enderEmit/xBairro',
                     C05_CMUN         VARCHAR2(7)  PATH 'infNFe/emit/enderEmit/cMun',
                     C05_XMUN         VARCHAR2(60) PATH 'infNFe/emit/enderEmit/xMun',
                     C05_UF           VARCHAR2(2)  PATH 'infNFe/emit/enderEmit/UF',
                     C05_CEP          VARCHAR2(8)  PATH 'infNFe/emit/enderEmit/CEP',
                     C05_CPAIS        VARCHAR2(4)  PATH 'infNFe/emit/enderEmit/cPais',
                     C05_XPAIS        VARCHAR2(60) PATH 'infNFe/emit/enderEmit/xPais',
                     C05_FONE         VARCHAR2(14) PATH 'infNFe/emit/enderEmit/fone',
                     C17_IE           VARCHAR2(14) PATH 'infNFe/emit/IE',
                     C18_IEST         VARCHAR2(14) PATH 'infNFe/emit/IEST',
                     C19_IM           VARCHAR2(15) PATH 'infNFe/emit/IM',
                     C20_CNAE         VARCHAR2(7)  PATH 'infNFe/emit/CNAE',
                     C21_CRT          VARCHAR2(1)  PATH 'infNFe/emit/CRT',

                     -- Destinatario
                     E02_CNPJ         VARCHAR2(14) PATH 'infNFe/dest/CNPJ',
                     E03_CPF          VARCHAR2(11) PATH 'infNFe/dest/CPF',
                     E03A_IDESTRANGEIRO VARCHAR2(20) PATH 'infNFe/dest/idEstrangeiro',
                     E04_XNOME        VARCHAR2(60) PATH 'infNFe/dest/xNome',
                     E05_XLGR         VARCHAR2(60) PATH 'infNFe/dest/enderDest/xLgr',
                     E05_NRO          VARCHAR2(60) PATH 'infNFe/dest/enderDest/nro',
                     E05_XCPL         VARCHAR2(60) PATH 'infNFe/dest/enderDest/xCpl',
                     E05_XBAIRRO      VARCHAR2(60) PATH 'infNFe/dest/enderDest/xBairro',
                     E05_CMUN         VARCHAR2(7)  PATH 'infNFe/dest/enderDest/cMun',
                     E05_XMUN         VARCHAR2(60) PATH 'infNFe/dest/enderDest/xMun',
                     E05_UF           VARCHAR2(2)  PATH 'infNFe/dest/enderDest/UF',
                     E05_CEP          VARCHAR2(8)  PATH 'infNFe/dest/enderDest/CEP',
                     E05_CPAIS        VARCHAR2(4)  PATH 'infNFe/dest/enderDest/cPais',
                     E05_XPAIS        VARCHAR2(60) PATH 'infNFe/dest/enderDest/xPais',
                     E05_FONE         VARCHAR2(14) PATH 'infNFe/dest/enderDest/fone',
                     E16_INDIEDEST    VARCHAR2(1)  PATH 'infNFe/dest/indIEDest',
                     E17_IE           VARCHAR2(14) PATH 'infNFe/dest/IE',
                     E18_ISUF         VARCHAR2(9)  PATH 'infNFe/dest/ISUF',
                     E19_IM           VARCHAR2(15) PATH 'infNFe/dest/IM',
                     E20_EMAIL        VARCHAR2(60) PATH 'infNFe/dest/email',

                     -- Grupo Z - Informacoes Adicionais
                     Z03_INFCPL       VARCHAR2(4000) PATH 'infNFe/infAdic/infCpl',

                     -- Grupo ZX - Informacoes Suplementares (QR Code - somente NFC-e mod 65)
                     ZX02_QRCODE      VARCHAR2(1000) PATH 'infNFeSupl/qrCode',

                     -- Passando no inteiro de itens para o Filho
                     XML_ITENS        XMLTYPE      PATH 'infNFe/det'
             ) capa,
             -- ==========================================
        ...
        -- Filtro rígido para garantir que apenas XMLs de lote entrem no cursor
        WHERE XMLExists('declare default element namespace "http://www.portalfiscal.inf.br/nfe"; /enviNFe' PASSING XMLTYPE(t.conteudo_xml));

    TYPE t_doc_array IS TABLE OF c_lote%ROWTYPE;
    ...
    BEGIN
        ...
        VALUES (p_chave, 'PRC_LOTE_NFE_REFORMA_CONSOLIDADA', p_nitem, p_cod, SUBSTR(p_msg, 1, 4000));
        COMMIT;
    END gravar_log;

BEGIN
    -- Limpa os dados de lote anteriores sem dar TRUNCATE geral
    DELETE FROM TB_REFORMA_CONSOLIDADA WHERE TIPO_DOCUMENTO = 'LOTE_NFE';
    COMMIT;

    OPEN c_lote;
    LOOP
        FETCH c_lote BULK COLLECT INTO v_doc_array LIMIT 1000;
    ...
    END LOOP;
    CLOSE c_lote;
END PRC_LOTE_NFE_REFORMA_CONSOLIDADA;
/
